...
iwd_dmenu() {
    main_menu() {
        while :; do
            ...
            if ! show_menu 'iwd' 'Connect' 'Disconnect' 'Forget'; then break; fi
        done
    }
    ...
    get_passphrase() { passphrase="$(fuzzel --dmenu  -p 'Passphrase:' --password)" ; }
    ...
    show_menu() {
        ...
        # print using dmenu
        if ! answer="$(printf '%s\n' "${options}" | fuzzel --dmenu -l 10 -p "${prompt}")"; then return 1; fi
